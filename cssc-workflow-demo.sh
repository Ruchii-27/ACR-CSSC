...
function create_continuous_patching_workflow(){
  ...
  az acr task timer update --name CSSC-ScanRegistryAndSchedulePatch --registry $ACR_REGISTRY --timer-name daily --schedule "$cronExpression" --enabled true --output json >> "$LOG_FILE" 2>&1

...
function create_continuous_patching_workflow(){
  echo_header "Creating continuous patching workflow..." 
  ...
  if [ $? -eq 0 ]; then
	  echo "Deployment successful. Check '$LOG_FILE' for details."
  else
	  echo "Deployment failed. Check '$LOG_FILE' for error messages."
  fi

  echo_separater
  echo "setting schedule for continuous patching.."  
  
  az acr task timer update --name cssc-trigger-workflow --registry $ACR_REGISTRY --timer-name daily --schedule "$cronExpression" --enabled true --output json >> "$LOG_FILE" 2>&1

  if [ $? -ne 0 ]; then
	  echo "Error while setting up cssc-continuous-patch timer. Check '$LOG_FILE' for details."
  fi
  
  az acr task update --name cssc-trigger-workflow -r $ACR_REGISTRY --status Enabled --output json >> "$LOG_FILE" 2>&1
...
function download_required_artifacts(){
  ...
  if [ -f "$acrfolder/$bicep_file_name" ]; then
	  ...
	  git clone -b csscworkflow $git_repo_url
	  # Change to the artifacts directory
  fi
}

function create_acr_vulnerability_workflow(){
  echo_header "Creating ACR vulnerability scan workflow..."
  echo ""
  ...
  request_body='{
	  ...
	  "properties": {
		...
		"step": {
		  "type": "EncodedTask",
		  "encodedTaskContent": "",
		  "values": []
		},
		"trigger": {
		  "baseImageTrigger": {
...
function create_acr_copacetic_workflow(){
  echo_header "Creating ACR copacetic workflow..."
  echo_separater
  echo "az acr cssc-workflow
	   --name acrcopatask
	   --type copa-patch-v1
...
function clean_registry_artifacts(){
	...
	az resource delete -g $RESOURCE_GROUP --id "/subscriptions/b4e7b127-622b-4b84-aab3-a1d0ca5b381d/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.ContainerRegistry/registries/$ACR_REGISTRY/tasks/cssc-patch-image" --api-version 2019-04-01
	az resource delete -g $RESOURCE_GROUP --id "/subscriptions/b4e7b127-622b-4b84-aab3-a1d0ca5b381d/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.ContainerRegistry/registries/$ACR_REGISTRY/tasks/cssc-scan-image" --api-version 2019-04-01
	az resource delete -g $RESOURCE_GROUP --id "/subscriptions/b4e7b127-622b-4b84-aab3-a1d0ca5b381d/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.ContainerRegistry/registries/$ACR_REGISTRY/tasks/cssc-trigger-workflow" --api-version 2019-04-01
...
function track_cssc_workflow(){
  ...
  echo_header "Tracking cssc workflow..."
  echo_separater
  ...
  echo_header "View logs for the runId: $runId"
  echo_separater
  echo "az acr cssc-workflow track-progress
	   --name csscdemoworkflow
	   --type patch
...
function initialize_cssc_workflow(){
  echo_header "Getting location of the registry"
  #acr_info="az acr show --name $ACR_REGISTRY" 
  #LOCATION=$($acr_info | jq -r '.location')

  LOCATION="eastus"
  echo "Registry location: $LOCATION"
}
